...
- `target.short_edge` selects a resolution tier: `360`, `480`, `540`, `720`, `768`, or `1080`. The aligned output dimensions are derived from it together with `target.aspect_ratio`. Use `768` for the released quality recipe — it is the only tier the published benchmarks and the `quality: "high"` Cache-DiT profile cover.
