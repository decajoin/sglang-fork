...
### Reference-image preprocessing and memory

Every Ref2VA reference **image** is resized to a 2048px short edge before it
reaches Qwen3-VL and the video VAE, upscaling small sources when needed. Shrinking
images client-side therefore saves nothing. Reference **videos** are unaffected by
this: they follow the request's `target.short_edge` tier.

At 2048px each reference image contributes roughly 4096 DiT tokens plus about as
many Qwen3-VL visual tokens, so reference-heavy requests can exhaust memory on
32 GB-class GPUs. `--minimax-h3-reference-image-short-edge` lowers that tier
server-wide; token count scales with its square, so 1024 costs a quarter of the
default and 1440 about half:

```bash Command
sglang serve \
  --model-path MiniMaxAI/MiniMax-H3 \
  --model-variant ref2va \
  --minimax-h3-reference-image-short-edge 1440
```

The value must be a positive multiple of 32. Leaving it unset keeps the released
2048px contract.

<Warning>
This is a departure from the checkpoint's audited preprocessing. It trades
conditioning fidelity — subject identity, fine texture, legible text in the
reference — for memory. Validate output quality at the tier you pick before
using it in production.
</Warning>

### Segment-sparse attention for reference-heavy requests

The DiT runs one non-causal attention over the whole packed sequence
`[text | reference | target audio | target video]`, so cost grows with the
square of the sequence length. A reference video is as long as the generated
video, which makes it the single largest contributor: for a 5s 768p job it
roughly doubles the sequence, and therefore quadruples attention work.

`--minimax-h3-segment-sparse-attn` stops **visual** reference rows from attending
to the generated target rows. References still condition the target — only their
own representation stops being refined by the tokens they condition — which
removes the reference-by-target quadrant of the score matrix. Measured on 12s
768p jobs, this cuts total request time by **12–14%** with a reference video and
**1–4%** with reference images (images contribute half their tokens through the
text segment, which is not restricted).

Reference **audio** deliberately keeps full attention. It is roughly 1% of the
reference rows, so its representation is almost entirely context-derived, and
restricting it measurably destroys soundtrack fidelity for ~1% of the saving.
...
The flag applies only to `ref2va` requests carrying a visual reference. FL2VA
keyframes anchor frames of the generated video rather than conditioning it from
outside, so they are never restricted; T2VA and audio-only ref2va have no visual
reference rows and the flag becomes a no-op. It is also ignored under
`--ring-degree`, whose contiguous row chunking is incompatible with splitting
attention by query band; `--ulysses-degree` works normally.

<Warning>
This changes the function the checkpoint was trained for — H3 was trained with
full bidirectional attention across the packed sequence. Validate output quality
before enabling it in production; the effect is largest on jobs whose references
carry fine detail that the model must resolve against the generated content.
</Warning>
...
- `target.short_edge` selects a resolution tier: `360`, `480`, `540`, `720`, `768`, or `1080`. It is optional and defaults to `768`, so requests that omit it keep the released geometry. The aligned output dimensions are derived from it together with `target.aspect_ratio`. Use `768` for the released quality recipe — it is the only tier the published benchmarks and the `quality: "high"` Cache-DiT profile cover.
- Each tier resolves the nominal short edge against its own soft area cap, then rounds both axes independently to the 32-pixel grid, so the effective short edge can land slightly below the requested tier (`720` at 16:9 resolves to 1280×704). Resolved canvases per tier:

  | `short_edge` | 21:9 | 16:9 | 4:3 | 1:1 | 3:4 | 9:16 |
  | --- | --- | --- | --- | --- | --- | --- |
  | 360 | 736×320 | 640×352 | 480×352 | 352×352 | 352×480 | 352×640 |
  | 480 | 960×416 | 832×480 | 640×480 | 480×480 | 480×640 | 480×832 |
  | 540 | 1088×480 | 960×544 | 704×544 | 544×544 | 544×704 | 544×960 |
  | 720 | 1440×608 | 1280×704 | 960×704 | 704×704 | 704×960 | 704×1280 |
  | 768 | 1536×672 | 1344×768 | 1024×768 | 768×768 | 768×1024 | 768×1344 |
  | 1080 | 2176×928 | 1920×1056 | 1440×1088 | 1088×1088 | 1088×1440 | 1056×1920 |

- Latency and memory scale with canvas area: the 1080 tier is roughly 2× the 768 tier and the 360 tier roughly 0.22×. Add every served size to `--warmup-resolutions`, otherwise the first request at each new tier pays the cold-start cost.
