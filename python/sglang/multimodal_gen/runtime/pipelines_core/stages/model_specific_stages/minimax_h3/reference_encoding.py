...
def minimax_h3_resolve_reference_image_shape(
    *,
    ...
    reference-image token count with the square of the ratio, which is the only
    reason to move off the default -- it is a departure from the checkpoint's
    audited preprocessing, so it trades conditioning fidelity for memory.
    """
